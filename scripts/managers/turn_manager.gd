...
func _ready() -> void:
	...
	#
	empty_special_card_slot.append($"../opp_card_slots/opp_slot")
	empty_special_card_slot.append($"../opp_card_slots/opp_slot2")
	empty_special_card_slot.append($"../opp_card_slots/opp_slot3")
...
func opp_turn():
	...
	$"../UI/end_turn".visible = false
	#
	turn_timer.start()
	await turn_timer.timeout
	#
	if $"../opp_deck".opp_deck.size() != 0:
		$"../opp_deck".draw_card()
		turn_timer.start()
		await turn_timer.timeout
...
func play_card_highest_points():
	...
	var opp_hand = $"../opp_hand".opp_hand
	if opp_hand.size() == 0:
		end_opp_turn()
		return
	...
	var card_with_highest_points = opp_hand[0]
	...
	#
	$"../opp_hand".remove_card_from_hand(card_with_highest_points)
	#
	end_opp_turn()
